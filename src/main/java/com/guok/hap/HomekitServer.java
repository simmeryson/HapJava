...
public class HomekitServer {
	 ...
	 * 
	 * @param authInfo authentication information for this accessory. These values should be persisted and re-supplied on re-start
	 * 	of your application.
	 * @param accessory accessory implementation. This will usually be an implementation of an interface in 
	 * 	{#link com.guok.hap.accessories com.guok.hap.accessories}. 
	...
	/**
	 * Creates a bridge accessory, capable of holding multiple child accessories. This has the advantage over multiple standalone
	 * accessories of only requiring a single pairing from iOS for the bridge.
	 * 
	 * @param authInfo authentication information for this accessory. These values should be persisted and re-supplied on re-start
	 * 	of your application.
	 ...
	 * the pin-code. You don't need to worry about that though - the salting is done on the plaintext pin. (Yes, plaintext
	 * passwords are bad. Please don't secure your nuclear storage facility with this implementation)
	 * 
	...
	/**
	 * Generates a value to supply in {@link HomekitAuthInfo#getPrivateKey() HomekitAuthInfo.getPrivKey()}. This is used as the
	 * private key during pairing and connection setup.
	 * 
	 * @return the generated key
	 * @throws InvalidAlgorithmParameterException if the JVM does not contain the necessary encryption algorithms.
	 */
	...
	/**
	 * Generates a value to supply in {@link HomekitAuthInfo#getMac() HomekitAuthInfo.getMac()}. This is used as the unique
	 * identifier of the accessory during mDNS advertising. It is a valid MAC address generated in the locally administered
	 * range so as not to conflict with any commercial devices.
	 * 
